...
> Gerado automaticamente para fechar gap spec-driven. Componente `components/ds/dashboard.tsx` estava sem spec em `docs/specs`. Base: template `docs/templates/component-spec.md`.

---

## Propósito

Componente `dashboard` — shell completo dashboard (AppSidebar + Header + SectionCards + Chart + DataTable).

**Usar quando:** compor páginas/layouts que precisam de dashboard.
...
| Campo      | Valor                                 |
| ---------- | ------------------------------------- |
| Arquivo    | `components/ds/dashboard.tsx`         |
| Tipo       | `registry:block`                      |
| Categoria  | `Layout`                              |
| Depende de | separator, sidebar, skeleton, tooltip |
