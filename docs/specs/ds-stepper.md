...
> Gerado automaticamente para fechar gap spec-driven. Componente `components/ds/stepper.tsx` estava sem spec em `docs/specs`. Base: template `docs/templates/component-spec.md`.

---

## Propósito

Componente `stepper` — wizard com validação per-step.

**Usar quando:** compor páginas/layouts que precisam de stepper.
...
| Campo      | Valor                       |
| ---------- | --------------------------- |
| Arquivo    | `components/ds/stepper.tsx` |
| Tipo       | `registry:ui`               |
| Categoria  | `Form`                      |
| Depende de | button, card                |
