...
> Gerado automaticamente para fechar gap spec-driven. Componente `components/ds/autocomplete.tsx` estava sem spec em `docs/specs`. Base: template `docs/templates/component-spec.md`.

---

## Propósito

Componente `autocomplete` — campo de busca com sugestões assíncronas e navegação por teclado.

**Usar quando:** compor páginas/layouts que precisam de autocomplete.
...
| Campo      | Valor                            |
| ---------- | -------------------------------- |
| Arquivo    | `components/ds/autocomplete.tsx` |
| Tipo       | `registry:ui`                    |
| Categoria  | `Form`                           |
| Depende de | input, popover, skeleton         |
