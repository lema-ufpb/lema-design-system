...
Dock flutuante com magnificação hover (desktop) e grid mobile — Aceternity Floating Dock. Evita colisão com header-* (que é top bar).
...
**Alternativa:** `tooltip` primitivos.
...
| Campo      | Valor                             |
| ---------- | --------------------------------- |
| Arquivo    | `components/ds/floating-dock.tsx` |
| Tipo       | `registry:ui`                     |
