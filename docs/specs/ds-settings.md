...
> Gerado automaticamente para fechar gap spec-driven. Componente `components/ds/settings.tsx` estava sem spec em `docs/specs`. Base: template `docs/templates/component-spec.md`.

---

## Propósito

Componente `settings` — tabs de configurações com seções.

**Usar quando:** compor páginas/layouts que precisam de settings.
...
| Campo      | Valor                        |
| ---------- | ---------------------------- |
| Arquivo    | `components/ds/settings.tsx` |
| Tipo       | `registry:ui`                |
| Categoria  | `Layout`                     |
| Depende de | card, skeleton, tabs         |
