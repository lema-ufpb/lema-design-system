...
Marquee duplo 2 rows com direção oposta, pause on hover e reduced-motion fallback — Launch UI Social Proof Marquee 2 Rows. Evita colisão com marquee (single row) e ticker.

**Usar quando:** compor landing com padrão Launch UI.
**Não usar quando:** hero/bento/marquee simples já resolve.
**Alternativa:** `card, avatar, skeleton` primitivos.
...
| Campo      | Valor                           |
| ---------- | ------------------------------- |
| Arquivo    | `components/ds/marquee-duo.tsx` |
| Tipo       | `registry:ui`                   |
| Categoria  | `Marketing`                     |
