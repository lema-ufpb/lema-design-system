...
Navbar flutuante (floating) com blur, border, rounded-2xl, sticky top-4 e menu mobile — Launch UI Navbar Floating/Centered. Evita colisão com header-* (fixed top sem floating).

**Usar quando:** compor landing com padrão Launch UI.
**Não usar quando:** hero/bento/marquee simples já resolve.
**Alternativa:** `button, skeleton` primitivos.
...
| Campo      | Valor                               |
| ---------- | ----------------------------------- |
| Arquivo    | `components/ds/navbar-floating.tsx` |
| Tipo       | `registry:block`                    |
| Categoria  | `Header`                            |
