...
export function TeamGrid({ className, title, description, members, columns = 3, locale = "en-US", loading = false, ...props }: TeamGridProps) {
  const t = UI_I18N[locale].team

  return (
    <div data-slot="team-grid" className={cn("flex flex-col gap-6", className)} {...props}>
      ...
      {loading ? (
        <div className={cn("grid gap-6", columns === 4 ? "md:grid-cols-2 lg:grid-cols-4" : "md:grid-cols-2 lg:grid-cols-3")}>
