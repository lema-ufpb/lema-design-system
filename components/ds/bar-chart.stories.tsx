    ...
    docs: {
      description: {
        component: [
          ...
          "### `data`",
          "",
          "An array of objects where each object represents a row or category. Every object key must be a `string` and every value must be a `string | number`.",
          "",
          "```tsx",
          "// Single series — one numeric field per object",
          "const data = [",
          ...
          "]",
          "",
          "// Multi series — two or more numeric fields per object",
          "const data = [",
          '  { dept: "Engineering", enrolled: 320, active: 285 },',
          '  { dept: "Medicine", enrolled: 180, active: 162 },',
          "]",
          "",
          "// String values are allowed — they render as-is on the category axis",
          "// and as labels on tooltips (no charting math is applied to strings).",
          "const data = [",
          '  { quarter: "Q1", status: "On track", budget: 120000 },',
          "]",
          "```",
          "",
          "If your backend returns numbers as strings, parse them to `number` before passing.",
          'Dates should be pre-formatted into human-readable strings (e.g. `"Jan"`, `"2025 Q1"`).',
          "",
          "> **Type:** `Record<string, string | number>[]`",
          "",
          "### `categoryKey`",
          "",
          "The object key used for the **category axis** — X axis on vertical orientation, Y axis on horizontal orientation. Each unique value in this field becomes a bar group.",
          "",
          "```tsx",
          'categoryKey="month"  // "Jan", "Feb", "Mar" → each gets a bar group',
          'categoryKey="dept"   // "Engineering", "Medicine" → each gets a bar group',
          "```",
          "",
          ...
          "### `dataKeys`",
          "",
          "Which object keys to render as bars. Accepts two shapes:",
          "",
          "**1. Plain strings** — uses the key as both the data accessor and the legend label:",
          "```tsx",
          'dataKeys={["revenue"]}',
          'dataKeys={["enrolled", "active"]}',
          "```",
          "",
          "**2. Objects `{ key, label?, color? }`** — full control over display and styling:",
          "```tsx",
          "dataKeys=[",
          ...
          "| --- | --- | --- | --- |",
          "| `key` | `string` | ✓ | Object key to read from each row in `data` |",
          "| `label` | `string` | — | Legend/tooltip label; falls back to `key` if omitted |",
          "| `color` | `string` | — | Any CSS color value (e.g. `var(--primary)`, `oklch(...)`, `#hex`); defaults to the next `--chart-N` token |",
          "",
          "> **Type:** `BarChartKey[] | string[]`  •  **Required**",
          "",
          "### Putting it together",
          "",
          "```tsx",
          ...
          '        { key: "forecast", label: "Forecast", color: "var(--chart-3)" },',
          "      ]}",
          "      height={300}",
          "    />",
          "  )",
          ...
          "| `dataKeys` | `BarChartKey[] \\| string[]` | — | (required) Keys to render as bars |",
          "| `categoryKey` | `string` | — | (required) Object key for the category axis |",
          "| `title` | `string` | — | Optional title |",
          "| `subtitle` | `string` | — | Optional subtitle |",
          "| `footer` | `React.ReactNode` | — | Optional footer |",
          '| `orientation` | `BarOrientation` | `"vertical"` | Bar direction |',
          ...
          "| `stacked` | `boolean` | `false` | Stack bars into a single column/row |",
          "| `barSize` | `number` | — | Fixed bar thickness in px |",
          "| `rounded` | `boolean` | `true` | Round the leading edge of each bar |",
          "| `valueFormatter` | `(value: number) => string` | — | Format the value axis ticks and tooltip |",
          "| `showBrush` | `boolean` | `false` | Range brush for scrolling and zooming the category axis |",
          ...
          "| `xAxisLabel` | `string` | — | Label centred on the category axis |",
          "| `yAxisLabel` | `string` | — | Label centred on the value axis |",
        ].join("\n"),
      },
    },
  },
