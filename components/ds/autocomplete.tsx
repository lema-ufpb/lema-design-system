...
export function Autocomplete({ className, options, value, onValueChange, placeholder = "Search...", loading = false, ...props }: AutocompleteProps) {
  ...
  const displayValue = value ? (options.find((o) => o.value === value)?.label ?? query) : query
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverAnchor asChild>
          <div className="relative">
            <SearchIcon className="pointer-events-none absolute left-3 top-1/2 z-10 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" />
            <Input
              role="combobox"
              aria-expanded={open}
              aria-controls={listboxId}
              aria-activedescendant={activeIndex >= 0 ? `${listboxId}-${filtered[activeIndex]?.value}` : undefined}
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter" && activeIndex >= 0) {
                  ...
                  if (opt) { onValueChange?.(opt.value); setQuery(opt.label); setOpen(false) }
                }
        ...
        <PopoverContent className="w-[var(--radix-popover-anchor-width)] p-1" align="start" onOpenAutoFocus={(e) => e.preventDefault()}>
          {loading ? (
            ...
            <div id={listboxId} role="listbox" className="flex flex-col gap-1">
              {filtered.map((opt, idx) => (
                ...
                >
                  ...
                  {value === opt.value && <CheckIcon className="size-3.5" aria-hidden="true" />}
                </button>
              ))}
            </div>
          )}
