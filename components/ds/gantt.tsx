...
export const ganttVariants = cva("w-full overflow-x-auto rounded-2xl border bg-card p-4", {
  variants: {
    size: {
      sm: "text-xs",
      md: "text-sm",
...
export function Gantt({ className, tasks, range, size = "md", loading = false, ...props }: GanttProps) {
  if (loading) {
    return (
      <div data-slot="gantt-skeleton" className={cn(ganttVariants({ size }), className)} {...props}>
  ...
  return (
    <div data-slot="gantt" className={cn(ganttVariants({ size }), className)} {...props}>
        ...
        {tasks.map((task) => {
          ...
          return (
            <div key={task.id} className="flex items-center gap-3">
              <span className="w-24 shrink-0 truncate text-xs font-medium text-muted-foreground">{task.name}</span>
