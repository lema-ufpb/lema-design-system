    ...
    docs: {
      description: {
        component: [
          ...
          "| --- | --- | --- |",
          "| `geoData` | `FeatureCollection \\| string` | Topology — GeoJSON object or URL path |",
          '| `featureIdProperty` | `string` | Property key used as feature ID when there is no top-level `id` (e.g. `"postal"` for Natural Earth state files) |',
          "| `data` | `GeoMapFeature[]` | Values / colors to overlay (choropleth) |",
          "| `markers` | `GeoMapMarker[]` | Point markers with coordinates |",
          "| `selectedFeatureIds` | `(string \\| number)[]` | Features to highlight with a stroke |",
          "| `colorRange` | `[string, string]` | Hex colors for min → max choropleth gradient |",
          "| `showLegend` | `boolean` | Show color scale legend — auto-enabled when `colorRange` is set |",
          '| `legendPosition` | `LegendPosition` | Where to render the legend (default `"bottom-right"`) |',
          '| `legendOrientation` | `LegendOrientation` | Gradient bar direction: `"horizontal"` (default) or `"vertical"` |',
          "| `enableZoom` | `boolean` | Scroll-to-zoom and drag-to-pan |",
          "",
          "## ID resolution order",
          "1. `feature.id` (top-level GeoJSON id)",
          "2. `feature.properties[featureIdProperty]` (when prop is set)",
          "3. `feature.properties.id` / `feature.properties.ID`",
          "",
