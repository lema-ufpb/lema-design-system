...
export const howItWorksStepNumberVariants = cva(
  "font-bold text-muted-foreground/20 tabular-nums",
  {
    variants: {
      size: {
        sm: "text-2xl",
...
export const howItWorksStepIconVariants = cva(
...
export function HowItWorks({
  ...
  if (loading) {
    return (
      <div
        data-slot="how-it-works-skeleton"
        className={cn(howItWorksVariants({ size }), className)}
        {...props}
      >
        ...
        <div
          className={cn(
            "grid gap-6",
            orientation === "horizontal" ? "md:grid-cols-3" : "grid-cols-1"
          )}
        >
        ...
        {steps.map((step, idx) => (
          ...
          >
            {variant === "number" ? (
              <span
                className={cn(
                  howItWorksStepNumberVariants({ size }),
                  "leading-none"
                )}
                aria-hidden="true"
              >
              ...
              <span
                className={cn(howItWorksStepIconVariants({ size }))}
                aria-hidden="true"
              >
                {step.icon ?? (
            ...
            {orientation === "horizontal" &&
              ...
              idx < steps.length - 1 && (
                <ArrowRightIcon
                  className="absolute top-6 -right-3 hidden size-4 text-muted-foreground/30 md:block"
                  aria-hidden="true"
                />
              )}
          </li>
