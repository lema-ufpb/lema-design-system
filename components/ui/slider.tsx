...
function Slider({
  ...
  // Accessible name forwarded from Root to each thumb — axe requires aria-input-field-name on role=slider
  const thumbLabel = (props["aria-label"] as string | undefined) ?? undefined
  const thumbLabelledBy = props["aria-labelledby"] as string | undefined
        ...
        <SliderPrimitive.Thumb
          ...
          aria-label={
            thumbLabelledBy
              ...
              : thumbLabel
                ? _values.length > 1
                  ? `${thumbLabel} ${index === 0 ? "minimum" : "maximum"}`
                  : thumbLabel
